...
describe("@mahiva/analyzer", () => {
  it("analyzes imports, exports, and symbols for a parsed TypeScript file", () => {
            ...
            children: [
              {
                ...
                range: {
                  start: { line: 4, column: 9, offset: 42 },
                  end: { line: 6, column: 10, offset: 72 },
                },
                children: [],
              },
            ],
    ...
    expect(result.symbols.map((symbol) => symbol.canonicalName)).toContain(
      "AuthService.login",
    );
  ...
  it("infers exported symbols from real parser output", async () => {
    ...
    expect(result.publicSymbols.map((symbol) => symbol.name)).toEqual(
      expect.arrayContaining(["User", "AuthService"]),
    );
